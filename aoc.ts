...
if (args.includes("--setup") || args.includes("-s")) {
	...
	if (!fs.existsSync(baseTemplatePath)) {
		logger.error(
			`Template folder ${chalk.underline(baseTemplatePath)} doesn't exist.`,
		);
		process.exit();
	}
...
logger.info(
	chalk.bold.underline(`--- Advent of Code ${year} - Day ${day} ---\n`),
);
...
for (const [partName, fileName] of Object.entries(parts)) {
	...
	if (!fs.existsSync(filePath)) {
		logger.error(
			`Couldn't find file ${chalk.underline(`${basePath}/${fileName}`)}`,
		);
		process.exit();
	}
	...
	if (
		answerQuestion.trim().toLowerCase() === "y" ||
		answerQuestion.trim() === ""
	) {
		...
		const answer = await submitAnswer(
			...
			year,
			day,
			partName.at(-1) ?? "not_found",
		);
		if (
			answer !== AnswerState.Correct &&
			answer !== AnswerState.AlreadySubmitted
		) {
			process.exit();
		}
	}

	const nextPart = Object.entries(parts).find(
		(part) => part[1] !== fileName,
	)?.[1];
	if (!nextPart) {
